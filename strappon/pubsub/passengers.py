...
class MatchedPassengerWithIdGetter(Publisher):
    def perform(self, repository, passenger_id):
        passenger = repository.get_matched_by_id(passenger_id)
        if passenger is None:
            self.publish('passenger_not_found', passenger_id)
        else:
            self.publish('passenger_found', passenger)
...
class PassengerLinkedToDriverWithIdAuthorizer(Publisher):
    def perform(self, driver_id, passenger):
        matching_requests = (driver_id == r.driver_id
                             for r in passenger.drive_requests)
        if any(matching_requests):
            self.publish('authorized', driver_id, passenger)
        else:
            self.publish('unauthorized', driver_id, passenger)
...
class MultiplePassengersUnmatcher(Publisher):
    def perform(self, passengers):
        def unmatch(p):
            p.matched = False
            return p
